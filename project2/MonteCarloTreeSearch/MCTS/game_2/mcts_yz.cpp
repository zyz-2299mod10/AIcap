...
/****GameState****/
GameState::GameState(int user_state[15][15], int sheep_state[15][15], char turn) : turn(turn) {
    for (int i = 0; i < 15; i++) {
        for (int j = 0; j < 15; j++) {
        ...
        for (int j = 0; j < 15; j++) {
            if (user_state[i][j] == turn && sheep_state[i][j] > 1) {
                for(int k = 0; k < 8; k++){
                    ...
                    if(x >= 0 && x < 15 && y >= 0 && y < 15 && user_state[x][y]  == '1'){
    ...
    for (int i = 0; i < 15; i++) {
        for (int j = 0; j < 15; j++) {
            ...
            for(int k = 0; k < 4; k++){
                ...
                if(x >= 0 && x < 15 && y >= 0 && y < 15 && user_state[x][y]  == '0'){ // 0 for wall
                    Action new_action = Action(i, j, 32, k);
...
void GameState::next_state(Action& action) {
    ...
    */
    ...
    while(1){
        ...
        if(new_x < 0 || new_x >= 15 || new_y < 0 || new_y >= 15 || user_state[new_x][new_y] != '1'){
            break;
        }
        x = new_x;
        y = new_y;
...
int GameState::recursive_calculate(int x, int y, char person){
    ...
    for(int i = 0; i < 4; i++){
        ...
        if(new_x >= 0 && new_x < 15 && new_y >= 0 && new_y < 15 && user_state[new_x][new_y] == person){
        ...
        for(int j = 0; j < 15; j++){
            ...
            if(state.user_state[i][j] == player_turn && state.sheep_state[i][j] > 1){
                for(int k = 0; k < 8; k++){
                    ...
                    if(x >= 0 && x < 15 && y >= 0 && y < 15 && state.user_state[x][y] == '1'){
                        direction++;
                    }
                }
                if(!direction)
